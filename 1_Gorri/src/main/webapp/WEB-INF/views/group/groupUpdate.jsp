...
<div class="aligncenterhae" style="height:800px; width:1000px; border:2px solid #ffab00; margin-left:auto; margin-right:auto; border-radius:2em; padding:90px;">
	...
	<form action="${ contextPath }/updateGroup.gr" method="post" enctype="multipart/form-data">
		<input type="hidden" value="${ membershipNo }">
		<table style="text-align:left;">
			<tr>
				<td rowspan="2">
					<img src="${ contextPath }/resources/uploadFiles/${a.renameName}"  class="gThumnail" id="groupPic">
					<input type="file" accept="image/*" class="form-control form-control-lg" name="file" id="updateGroupPic" onchange="setThumbnail(event);">
					<div id="showPic" style="display:none;" required>					
					</div>
				</td>
			...
			<table>
				...
				<tr>
					<td colspan="4" style="padding-left:30px; padding-right:10px; padding-top:10px;">
						<input name="membershipContent" class="form-control me-2" value="${ groupInfo.membershipContent }" style="width:100%; border: 1px solid lightgrey;" required>
					</td>
				</tr>
			</table>
		</div>
...
<script>
	...
	window.onload=()=>{		
		...
		const groupPic = document.getElementById('groupPic')
        ...
        reader.onload = (event)=> {
          const updateImg = document.getElementById("groupPic")
